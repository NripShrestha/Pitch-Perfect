...
import pitchperfect.bean.InvoiceBean;
...
public class InvoiceModel {
	...
	public Integer nextPK() throws DatabaseException {
		...
		try {
			...
			PreparedStatement pstmt = conn.prepareStatement("SELECT MAX(ID) FROM INVOICE");
	...
	public Integer nextProductCode() throws DatabaseException {
		...
		try {
			conn = JDBCDataSource.getConnection();
			PreparedStatement pstmt = conn.prepareStatement("SELECT MAX(OrderId) FROM INVOICE");
			ResultSet rs = pstmt.executeQuery();
			while (rs.next()) {
				pk = rs.getInt(1);
			}
	...
	public InvoiceBean findByPK(long pk) throws ApplicationException {
		...
		StringBuffer sql = new StringBuffer("SELECT * FROM INVOICE WHERE ID=?");
	...
	public InvoiceBean findByOrderId(long pk) throws ApplicationException {
		log.debug("Model findByPK Started");
		StringBuffer sql = new StringBuffer("SELECT * FROM INVOICE WHERE ORDERID=?");
		InvoiceBean bean = null;
		Connection conn = null;
		try {
			conn = JDBCDataSource.getConnection();
	...
	public long add(InvoiceBean bean) throws ApplicationException, DuplicateRecordException {
	 ...
	 public List search(InvoiceBean bean) throws ApplicationException {
	    ...
	    public List list(int pageNo, int pageSize) throws ApplicationException {
	        ...
	        ArrayList list = new ArrayList();
	        StringBuffer sql = new StringBuffer("select * from INVOICE");
	        // if page size is greater than zero then apply pagination
	        if (pageSize > 0) {
	            // Calculate start record index
	            pageNo = (pageNo - 1) * pageSize;
