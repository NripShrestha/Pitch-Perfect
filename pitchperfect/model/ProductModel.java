...
public class ProductModel {
	...
	public Integer nextProductCode() throws DatabaseException {
		...
		try {
			conn = JDBCDataSource.getConnection();
			PreparedStatement pstmt = conn.prepareStatement("SELECT MAX(productCode) FROM PRODUCT");
			ResultSet rs = pstmt.executeQuery();
			while (rs.next()) {
				pk = rs.getInt(1);
			}
	...
	public ProductBean findByCategoryAndName(long catId, String name) throws ApplicationException {
		...
		StringBuffer sql = new StringBuffer("SELECT * FROM PRODUCT WHERE categoryId=? And Name=?");
		ProductBean bean = null;
		Connection conn = null;
		try {
			conn = JDBCDataSource.getConnection();
	...
	public ProductBean findByProductCode(long pk) throws ApplicationException {
		...
		StringBuffer sql = new StringBuffer("SELECT * FROM PRODUCT WHERE ProductCode=?");
		ProductBean bean = null;
		Connection conn = null;
		try {
			conn = JDBCDataSource.getConnection();
	 ...
	 public List search(ProductBean bean) throws ApplicationException {
	    ...
	    public void update(ProductBean bean) throws ApplicationException,
	        ...
	        try {
	            ...
	            conn.setAutoCommit(false); // Begin transaction
	            PreparedStatement pstmt = conn
	                    .prepareStatement("UPDATE PRODUCT SET CATEGORYID=?,CATEGORYNAME=?,PRODUCTCODE=?, NAME=?,DESCRIPTION=?,PRICE=?,IMAGE=?,CREATEDBY=?,MODIFIEDBY=?,CREATEDDATETIME=?,MODIFIEDDATETIME=? WHERE ID=?");
