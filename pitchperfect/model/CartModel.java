...
import pitchperfect.bean.CartBean;
...
import pitchperfect.exception.DatabaseException;
...
public class CartModel {
	...
	public Integer nextPK() throws DatabaseException {
		...
		try {
			...
			PreparedStatement pstmt = conn.prepareStatement("SELECT MAX(ID) FROM CART");
	...
	public CartBean findByName(String name) throws ApplicationException {
		...
		StringBuffer sql = new StringBuffer("SELECT * FROM CART WHERE PRODUCTNAME=?");
		CartBean bean = null;
		Connection conn = null;
		try {
			conn = JDBCDataSource.getConnection();
	...
	public CartBean findByPK(long pk) throws ApplicationException {
		...
		StringBuffer sql = new StringBuffer("SELECT * FROM CART WHERE ID=?");
		CartBean bean = null;
		Connection conn = null;
		try {
			conn = JDBCDataSource.getConnection();
	...
	public CartBean findByproIdAndUserId(long proId, long UserId) throws ApplicationException {
		log.debug("Model findByPK Started");
		StringBuffer sql = new StringBuffer("SELECT * FROM CART WHERE productId=? And userId=?");
		CartBean bean = null;
		Connection conn = null;
		try {
			conn = JDBCDataSource.getConnection();
	...
	public List search(CartBean bean, int pageNo, int pageSize) throws ApplicationException {
		...
		StringBuffer sql = new StringBuffer("SELECT * FROM CART WHERE 1=1");
		if (bean != null) {
			if (bean.getId() > 0) {
	...
	public List list(int pageNo, int pageSize) throws ApplicationException {
		...
		ArrayList list = new ArrayList();
		StringBuffer sql = new StringBuffer("select * from CART");
		// if page size is greater than zero then apply pagination
		if (pageSize > 0) {
			// Calculate start record index
			pageNo = (pageNo - 1) * pageSize;
	...
	public void update(CartBean bean) throws ApplicationException, DuplicateRecordException {
		...
		try {
			...
			PreparedStatement pstmt = conn.prepareStatement(
					"UPDATE Cart SET UserId=?,UserName=?,ProductId=?,PruductName=?,Quantity=?,FinalAmount=?,CREATEDBY=?,MODIFIEDBY=?,CREATEDDATETIME=?,MODIFIEDDATETIME=? WHERE ID=?");
